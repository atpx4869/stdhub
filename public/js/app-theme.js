...
(function() {
  ...
  var KEY = 'bzxz.theme';
  var VALID = ['paper', 'legacy'];

  var THEME_META = {
    paper:  { iconClass: 'ti-file-text', label: 'Paper' },
    legacy: { iconClass: 'ti-diamond', label: '经典（老设备）' },
  };

  function getTheme() {
    try {
      var t = localStorage.getItem(KEY);
      return VALID.indexOf(t) >= 0 ? t : 'paper';
    } catch (e) { return 'paper'; }
  }

  function setTheme(theme) {
    if (VALID.indexOf(theme) < 0) theme = 'paper';
  ...
  function toggleTheme() {
    var cur = getTheme();
    setTheme(cur === 'legacy' ? 'paper' : 'legacy');
  }
  ...
  function syncThemeUI() {
    var theme = getTheme();
    var meta = THEME_META[theme] || THEME_META.paper;
    // 「我」页 chip
