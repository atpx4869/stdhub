...
- 标准 PDF 预览与下载；自动入库时显示查库、搜源、下载、保存入库等阶段反馈，并在页面空闲时预热 PDF 渲染资源
...
- 标准号类查询优先走归一化索引快路径，并按精确匹配、同系列 / 跨年提示、全文匹配分区展示；普通资质搜索支持加载更多，按标准查展开后再加载机构明细
- 资质可视化（批量关键词查询）
- 订阅管理与自动同步
...
- PDF.js / pdfh5 预览渲染（支持双指缩放/双击切换适配，大文件优先提示等待）
- 下拉刷新（搜索结果 + 文件库）
