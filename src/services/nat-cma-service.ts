...
export const NAT_CMA_SUSPENDED_MESSAGE = '国家 CMA 已无限期暂停；历史数据仅供只读查询';
...
export class NationalCmaProviderUnavailable implements NatCmaProvider {
  async scrapeFull(): Promise<{ detail: NatCmaDetail; capabilities: NatCmaCapability[] }> {
    throw new Error(NAT_CMA_SUSPENDED_MESSAGE);
  }
}
...
export class NatCmaService {
  ...
  search(query: string, options: { limit?: number; offset?: number } = {}) {
    ...
    return {
      readOnly: true,
      suspended: true,
      message: NAT_CMA_SUSPENDED_MESSAGE,
      total,
      items: rows.map(row => ({
  ...
  getStatus() {
    ...
    return {
      ...
      suspended: true,
      readOnly: true,
      source: '国家 CMA（cma.cnca.cn）历史数据',
      abilityScope: 'organization',
      providerReady: false,
      providerMessage: NAT_CMA_SUSPENDED_MESSAGE,
    };
  }
  ...
  private get providerMessage(): string | null {
    return this.providerReady ? null : NAT_CMA_SUSPENDED_MESSAGE;
  }
  ...
  private ensureTables(): void {
    ...
    if (!subsColumns.has('cert_status')) {
      this.db.exec('ALTER TABLE nat_cma_subscriptions ADD COLUMN cert_status TEXT');
    }
    if (!subsColumns.has('cert_issued_at')) {
      this.db.exec('ALTER TABLE nat_cma_subscriptions ADD COLUMN cert_issued_at TEXT');
    }
    if (!subsColumns.has('cert_valid_from')) {
      this.db.exec('ALTER TABLE nat_cma_subscriptions ADD COLUMN cert_valid_from TEXT');
    }
    if (!subsColumns.has('cert_valid_to')) {
      this.db.exec('ALTER TABLE nat_cma_subscriptions ADD COLUMN cert_valid_to TEXT');
    }
