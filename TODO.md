...
#### 国家 CMA 功能
- [ ] **优先：国家 CMA 现行证书编号校准与安全迁移**：2026-07-16 真实抓取确认 publicDetailId `LI201581410348LI5860` 返回现行证书 `221700110366`（有效期 2026-03-12 至 2028-09-18），当前内置订阅仍使用 `230020349767`；迁移时需同时审查并更新订阅与能力缓存，不能直接覆盖旧数据
- [x] 国家 CMA 同步可靠性：按证书同步锁、事务替换缓存、能力唯一源标识、重启中断状态恢复
- [x] 国家 CMA 定时同步：纳入现有资质同步计划（按订阅证书去重，不重复抓取多个场所）
- [x] 国家 CMA 任务反馈：任务中心按真实成功/失败状态结束；页面明确为机构级能力数据
- [x] 国家 CMA 资质搜索：设置页支持按标准号、检测方法、产品或领域检索本地机构级能力并分页
- [x] 国家 CMA 资质标识匹配：标准检索、LABR、本地文件库与资质查询页显示严格同年版的国家 CMA 机构级徽章
