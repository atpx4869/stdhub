...
- [ ] 资质查询标准号快路径：标准号类输入优先走 `std_code_norm/std_code_base` 索引，首屏先显示精确命中，再补全文匹配
- [ ] 资质结果分区展示：精确匹配、同系列/跨年提示、全文匹配分区展示，避免不同年份资质误读为严格命中
- [ ] 按标准查延迟加载：先返回标准组摘要，用户展开后再加载机构明细，避免大产品标准一次性拉取大量行
- [ ] 资质查询分页 / 加载更多：明细查询支持 offset 或 cursor，减少一次性渲染压力
- [ ] 预览资源加载优化：空闲预加载 PDF.js/pdfh5；大文件或页数多时提示优先用浏览器原生打开
- [ ] 预览原生打开兜底（最后实施）：预览弹层、手机失败页和长时间等待状态增加“用浏览器原生打开”；华为手机等内嵌 PDF 不兼容时可直接跳系统浏览器或下载后打开
