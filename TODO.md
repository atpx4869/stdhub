...
#### 国家 CMA 功能
- [ ] **优先：接入真实国家 CMA（`cma.cnca.cn`）提供者**：当前已确认不能使用 CMA 实验室公共查询源（`223.75.53.51:81`）代替；需采用官方稳定接口、授权 API 或人工验证会话
- [x] 国家 CMA 公开入口可达性验证：2026-07-16 `https://cma.cnca.cn/cma/solr/tBzAbilitySearch/list` 返回 HTTP 200 并下发 `JSESSIONID`；未请求验证码、未抓取能力数据
- [ ] 国家 CMA 真实抓取验证：公开入口已验证可达；场所与能力接口需要人工完成滑块验证取得 `finalX`，后续仅在授权/人工验证会话下做只读字段与分页验证
- [ ] 国家 CMA 同步可靠性：在真实提供者接入后启用按证书同步锁、事务替换缓存、重启中断状态恢复
- [ ] 国家 CMA 定时同步：真实提供者稳定后再纳入现有资质自动同步计划
- [ ] 国家 CMA 资质搜索与标识匹配：真实数据入库后恢复本地搜索、严格同年版徽章与分页
- [ ] 国家 CMA 与 CMA 一单一库联动：在国家 CMA 机构级检索结果中展示一单一库状态、领域与年版提示
- [ ] 国家 CMA 与 CNAS/CMA 本地订阅结果的统一对比视图：按标准号聚合来源，但保留机构级 / 场所级数据范围
- [ ] 国家 CMA 抓取健康检查：设置页显示最近真实抓取时间、记录数与上游错误摘要
- [ ] 官方场所级数据接入：仅在有稳定官方接口、授权 API 或人工验证会话后实施；不接入验证码绕过逻辑
