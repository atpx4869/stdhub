...
```text
网络/超时：30s → 120s
429/503：30s → 120s（避免固定两来源串行时长时间占用全局同步锁）
反爬：页面内 1.5s × 3；周期级等待 60s 后再试一次
浏览器关闭：立即重建一次
```
...
Phase 0–2 已在第一批落地：固定机构事实由常量与测试锁定；设置页收敛为“资质数据”；新增固定 profile/status/sync 接口；自动资质周期只同步固定两来源；异常空结果/记录骤降会被安全闸门拒绝并保留旧快照。第二批已实施 Phase 3–4 的错误分类、限次退避和调度互斥队列，并清理不可达的多机构前端函数及两个死配置。多机构后端旧接口与关联表仍保留兼容期，待稳定后再做不可逆清理。
