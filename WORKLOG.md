...
### 2026-07-16：国家 CMA 真实抓取验证
- 使用独立临时进程直连公开查询源，未写入正式 SQLite 数据库；`LI201581410348LI5860` 抓取成功，耗时约 31.9 秒，返回 28,110 条能力记录。
- 机构名称为“湖北省产品质量监督检验研究院”，状态“正常”。
- 发现配置校准问题：公开页面现行证书号为 `221700110366`，发证日/有效期起为 2026-03-12，有效期止为 2028-09-18；现有内置订阅号为 `230020349767`。已列为优先迁移任务，待审查已有订阅和缓存后处理。
