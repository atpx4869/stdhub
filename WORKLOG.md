...
### 2026-09-08 维护路线 D3-D6 收尾

- 游客可预览已入库标准：开放本地命中、分页图片与在线 PDF 查看；自动下载入库、原件下载、重试和文件管理仍限管理员。
- 文件重命名、删除与下载入库增加数据库/文件系统补偿，并在启动扫描清理陈旧删除墓碑。
- 建立 `StdHub` 前端公共层，核心链路统一请求、生命周期、弹窗和资源加载；122 个 inline handler 改为事件委托并启用 CSP，Chart.js 改为按需加载。
- 环境配置改为 Zod 集中校验；适配器来源与文件库来源分型；下载、诊断、后台生命周期和数据库迁移从应用装配中拆出。
- 删除遗留多用户管理 API/脚本，固化“游客 + 单管理员”NAS 模式；`data/cma_national.db` 仅从 Git 移除，本机历史文件保留。
- 发布改为手动触发、先验证后发版，新增 Chromium E2E、Docker smoke 与高危漏洞阻断；生产依赖审计为 0 漏洞。

### 2026-09-07 默认管理员登录修复

- 修复新数据库未配置 `STDHUB_ADMIN_PASSWORD` 时不创建管理员、导致默认账号无法登录的问题。
- 首次启动现在自动创建 `admin / adminadmin`；环境变量仍可覆盖初始密码，且不会覆盖已有管理员密码。
- 新增默认密码、环境变量覆盖和已有密码保护三类回归测试，并补充部署文档与首次登录改密提示。

### 2026-09-05 游客只读与单管理员解锁

- 完成 guest/admin 会话、管理员密码登录/初始化/退出、滑动续期、过期降级与失败限速。
- 管理员写请求增加 Origin/Referer 与双提交 CSRF 校验；会话只保存哈希令牌。
- 收紧后端路由：游客仅可访问标准检索、资质查询、CMA 能力库搜索和工具箱公开查询；Labr、文件库管理、订阅/同步、查新、下载/导出、任务、运维均要求管理员。
- 前端按身份切换桌面/手机导航，隐藏 Labr、CMA 机构维度对比、领域订阅与任务/统计入口；管理员登录后无需刷新恢复完整菜单。
- 验证：TypeScript 构建通过，24 个测试文件共 219 项测试通过。
